...
export default {
    ...
    setup() {
        ...
        const searchItems = async () => {
            ...
            if (query.value.trim() !== '') {
                const movieResponse = await fetch(
                    `https://api.themoviedb.org/3/search/movie?api_key=${state.apiKey}&query=${query.value}&language=it-IT`
                );
                const movieData = await movieResponse.json();

                const tvResponse = await fetch(
                    `https://api.themoviedb.org/3/search/tv?api_key=${state.apiKey}&query=${query.value}&language=it-IT`
                );
        ...
        const getPosterUrl = (posterPath) => {
            ...
            const size = 'w342'; // Puoi cambiare la dimensione se desideri
